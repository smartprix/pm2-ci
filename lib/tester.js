...
class Tester {
	...
	async _runTests(manualHook = false, retry = 1) {
		...
		try {
			...
			logger.error('[%s] [%s] Can\'t copy report, error: %s', this.appName, this.commit, error);
			logger.error('[%s] [%s] Error for tests: %s', this.appName, this.commit, err);
		}
	...
	async _gitBisect() {
		if (this.lastGoodCommit === await this.repo.getHead().hash) {
			...
			throw new Error(`[${this.appName}] [${this.commit}] Lastgoodcommit is same as latest failing commit`);
		}
		else if (!this.lastGoodCommit) {
			// TODO: goodCommit = HEAD~50 or first whichever is recent
			throw new Error(`[${this.appName}] [${this.commit}] No lastGoodCommit defined`);
		}
	...
	*/
	...
	async sendSlack(res) {
		...
		try {
			...
			logger.error('[%s] [%s] Error: %s', this.appName, this.commit, e);
		}
