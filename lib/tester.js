...
class Tester {
	...
	async _getTest(commitHash) {
		let report;
		try {
			report = await fse.readFile(this.vars.testDestFile(commitHash), 'utf8');
	...
	*/
	async _getCoverage(commitHash) {
		let coverage;
		try {
			coverage = await fse.readFile(this.vars.coverageDestFile(commitHash), 'utf8');
	 ...
	 * @returns {TestResults}
	 */
	async _runTests(manualHook = false, retry = 0) {
		let err;
		...
		try {
			...
			await fse.remove(this.vars.covergeSrcDir);
		}
		catch(er) {
			// 
		}
