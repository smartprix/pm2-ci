...
class Tester {
	...
	*/
	async test({manualHook = false, sendSlack = true}) {
		...
		// Tests Failed
		if (!res.pass) {
			logger.error('[%s] [%s] Tests failed for app, failures: ', this.appName, this.commit, res.report.failures);
	...
	async sendSlack(res) {
		...
		if (res.pass) return slack.send(`Tests Passed for app *${this.appName}*`, attachments);
