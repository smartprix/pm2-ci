...
class Tester {
	...
	async _runTests(manualHook = false, retry = 1) {
		...
		process.env.CFG__PORT = await Tester.getFreePort();

		// TODO: get private config path from git.repo_path (VersioningInfo)
		if (this.app.privateConfig) {
			await fse.copy(path.join(this.projectCwd, this.app.privateConfig).toString(), `${this.path}/${this.app.privateConfig}`);
		}

		// Run tests
		try {
			await spawnAsExec(this.testCommand, this.appName, {
				cwd: this.path,
				env: process.env,
				// maxBuffer: 500 * 1024,
			});
	...
	async _gitBisect() {
		if (await this._getLastGoodCommit() === (await this.repo.getHead()).hash) {
			...
			throw new Error(`No lastGoodCommit defined, failing commit is ${this.commit}`);
		}
