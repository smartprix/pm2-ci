...
class Tester {
	...
	async test() {
		const obj = await this._runTests(this.head.hash);
		this.result = obj.tests;
		...
		// Tests Failed
		if (!obj.pass || this.result.code || this.report.failures) {
