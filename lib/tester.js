...
async function startGitBisect(repo, lastGoodCommit, testOpts) {
	...
	const loop = io.stdout.match(/roughly (\d+) step/)[1];
	...
	logger.log(`Git bisect result ${JSON.stringify(io)}`);
	return Git.Commit.lookup(repo, io.stdout.match(/(\S+)\s.+/)[1]);
}
...
async function testRunner({git, app, appName, reportsDir}) {
	...
	if (tests.code || report.failures) {
		...
		const firstBadCommit = await startGitBisect(repo, app.tests.lastGoodCommit, {
			cmd: testCommand,
			reportSrc,
			reportsDir,
			path: tmpDir.path,
		});
		res.commit = firstBadCommit.sha();
