...
class Tester {
	...
	constructor(git, opts, appName) {
		...
		this.repoDirName = 'repo';
		this.coverageDirName = 'coverage';
		this.testDirName = 'testReport';
		return this;
	}
	...
	get dirPath() {
		return `${this.opts.dataDir}/${this.appsDirName}/${this.appName}`;
	}

	get path() {
		return this.dirPath + '/' + this.repoDirName;
	}
	...
	get testSrcFolder() {
		return `${this.path}/${this.testDirName}`;
	}

	get testDestFolder() {
		return `${this.dirPath}/${this.testDirName}/`;
	}

	get covergeSrcFolder() {
		return `${this.path}/${this.coverageDirName}`;
	}

	get coverageDestFolder() {
		return `${this.dirPath}/${this.coverageDirName}/`;
	}
	...
	static get coverageFile() {
		return 'coverage-summary.json';
	}

	static get coverageHtml() {
		return 'index.html';
	}
	...
	async _getReport(commit) {
		let report;
		try {
			report = await fse.readFile(`${this.testDestFolder}/${commit.hash}/${this.testFile}`, 'utf8');
	 ...
	 *
	*/
	async _getCoverage(commit) {
		let coverage;
		try {
			coverage = await fse.readFile(`${this.coverageDestFolder}/${commit.hash}/${Tester.coverageFile}`, 'utf8');
			coverage = JSON.parse(coverage).total;
			coverage.url = new URL(`${this.appName}/${commit.hash}/coverage/${Tester.coverageHtml}`, this.hostUrl).href;
		}
		catch (e) {
			coverage = {failures: 1};
		}
	...
	async _runTests(manualHook = false, retry = 1) {
		...
		// Copy report
		try {
			await fse.ensureDir(`${this.testDestFolder}/${commit.hash}`);
			await fse.copy(
				`${this.testSrcFolder}/${this.testFile}`,
				`${this.testDestFolder}/${commit.hash}/${this.testFile}`,
				{overwrite: true}
			);
			console.log(`${this.testDestFolder}/${commit.hash}/${this.testFile}`);
			await fse.copy(
				`${this.testSrcFolder}/${this.testHtml}`,
				`${this.testDestFolder}/${commit.hash}/${this.testHtml}`,
				{overwrite: true}
			);

			await fse.ensureDir(`${this.coverageDestFolder}/${commit.hash}`);
			await fse.copy(
				`${this.covergeSrcFolder}`,
				`${this.coverageDestFolder}/${commit.hash}`,
				{overwrite: true}
			);
		}
		catch (error) {
		...
		const totalCoverage = await this._getCoverage(commit);
