...
class Tester {
	...
	get remoteUrl() {
		return this.git.remoteUrl || this.remote.url().slice(0, this.remote.url().indexOf('.git')) + '/';
	}
	...
	async _getReport() {
		...
		try {
			...
			report = require(this.reportPath).stats;
		}
		catch (e) {
			report = {};
		}
	...
	async test() {
		...
		if (this.result.code || this.report.failures) {
			logger.log(`Tests failed for app ${this.appName} on latest commit, ${this.head.short}`);
