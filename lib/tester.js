...
class Tester {
	...
	constructor(git, opts, appName) {
		...
		if (this.app.git) {
			git.token = this.app.git.token;
			git.remoteUrl = this.app.git.remoteUrl;
		}
	...
	_getDb(commit) {
		return db.get(`${this.appName}.bisectHistory.${commit.hash}`).value();
	}

	_setDb(commit, value) {
		return db.set(
			`${this.appName}.bisectHistory.${commit.hash}`,
			value
		).write();
	}
	...
	async _runTests() {
		...
		if (this._getDb(commit) !== undefined) {
			return this._getDb(commit);
		}
		...
		if (report.skipped === 0) {
			this._setDb(commit,	{commit, err, report, pass});
		}
