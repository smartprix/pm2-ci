...
class Tester {
	...
	get testCommand() {
		...
		return `${this.app.tests.testCmd} -- --reporter mochawesome --reporter-options ` +
			...
			`reportPageTitle=${this.appName + '-' + (this.repo.head ? this.repo.head.short : '')},` +
			'showPassed=false,' +
			'showPending=true,' +
			'showSkipped=true,' +
			'saveJson=true,' +
	...
	async _runTests(manualHook = false, retry = 1) {
		...
		if (report.skipped === 0 || retry === 0) {
			...
			logger.log(`Retrying tests on app ${this.appName} on commit ${commit.short} due to ${report.skipped} skipped tests`);
	...
	*/
	async test(manualHook = false) {
		...
		// Tests Failed
		if (!res.pass) {
			logger.error(`Tests failed for app ${this.appName} on commit, ${res.commit.short}`);
