...
class GitExtended {
	...
	async init(localPath) {
		// make remote url
		console.log('repo dir - ', localPath);
		const match = (this.opts.remoteUrl || this.opts.url).match(/(github.com)[/:](.*)/);
	 ...
	 * @param {String} remoteUrl remote url
	 */
	static async _clone(localPath, remoteUrl) {
		try {
	 ...
	 */
	static async _checkOut(localPath, what) {
		console.log('checkout ', what);
		try {
			await git(localPath).checkout(what);
		}
		catch (error) {
	...
	async pullOrClone() {
		...
		console.log('PullOrClone', pathExists, this.localPath);
		if (pathExists) {
			try {
				await GitExtended._pull(this.localPath, this.remoteUrl, this.branchName);
			}
			catch (error) {
				if (pathExists) await fse.remove(this.localPath);
				await GitExtended._clone(this.localPath, this.remoteUrl);
			}
		}
		else {
			await GitExtended._clone(this.localPath, this.remoteUrl);
	...
	async checkout(commit) {
		...
		if (typeof commit === 'object' && commit.hash) {
			...
			throw new Error('checkout commit failed');
		}
	}
