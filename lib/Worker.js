...
class Worker {
	...
	handleRequest(ctx, manualHook = false) {
		...
		this.processRequest(targetApp, targetName, git, manualHook);
	}
	 ...
	 * @param {Boolean} manualHook
	 */
	processRequest(targetApp, targetName, git, manualHook) {
		const execOptions = {
			cwd: targetApp.cwd,
			env: process.env,
			shell: true,
		...
		const phases = {
			...
			testRunner: async () => {
				...
				try {
					...
					logger.log('Starting tests for application %s, on commit %s', targetName, (await tester.repo.getHead()).short);
					res = await tester.test(manualHook);
				}
				catch (e) {
					logger.error(e);
				}
				...
				if (!res.pass && targetApp.tests.deployAnyway) {
					logger.error(`Tests failed for app ${targetName} on commit ${res.commit && res.commit.short}, ` +
					`found bad commit: ${res.bisect && res.bisect.commit && res.bisect.commit.short}. Deploying anyway due to config`);
					return;
				}
				else if (!res.pass) {
					throw new Error(`Tests failed for app ${targetName} on commit ${res.commit && res.commit.short}, ` +
					`found bad commit: ${res.bisect && res.bisect.commit && res.bisect.commit.short}`);
				}
				logger.log('All tests passing on commit, %s for application %s, pulling', res.commit.short, targetName);
	...
	async start() {
		return new Promise((resolve) => {
			this.server.listen(this.port, () => {
				logger.log('Server is ready and listening on port %s', this.port);
				resolve(this);
			});
		});
	}
