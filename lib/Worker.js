...
class Worker {
	 ...
	 */
	processRequest(req, checkRequest = true) {
		...
		const error = checkRequest ? this.checkRequest(targetApp, req) : false;
		if (error) {
		...
		const phases = {
			...
			testRunner: async () => {
				...
				let res = {};
				try {
					// TODO: use the same tmp dir and repo for all testing of this app
					const tester = await new Tester(versioning, this.opts, targetName).init();
					logger.log('Initialised tmpdir for testing of app ', targetName);
				...
				logger.write('Full result:', res.err);
		...
		async.series(Object.keys(phases).map(k => phases[k]),
			(err) => {
				if (err) {
					...
					logger.error('App : %s\nError Code: %s (full error in file logs)', targetName, err.code);
					logger.write(`Full Error:\n ${JSON.stringify(err)}`);
				}
			}
		);
	}
