			...
			testRunner: async () => {
				...
				if (!res.pass && targetApp.tests.deployAnyway) {
					...
					if (targetApp.tests.lastGoodCommit !== res.commit.hash &&
						(!opts.manualHook || opts.deploy)
					) {
						logger.log('[%s] [%s] Updating lastGoodCommit', targetName, res.commit.short);
						...
						await new Promise((resolve, reject) => {
							pm2.set('pm2-githook2:apps', this.apps, (err) => {
								if (err) reject(err);
								else resolve();
							});
						});
