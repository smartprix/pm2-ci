		...
		const phases = {
			...
			testRunner: async () => {
				...
				try {
					...
					logger.log('[%s] [%s] Cloned repo to tmpdir for testing of app', targetName, (await tester.repo.getHead()).short);

					logger.log('[%s] [%s] Starting tests for application', targetName, (await tester.repo.getHead()).short);
				...
				logger.log('[%s] [%s] All tests passing, pulling', targetName, res.commit.short);
