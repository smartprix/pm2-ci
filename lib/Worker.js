...
class Worker {
			...
			sendSlack: async () => {
				...
				if (res !== undefined) {
					...
					attachments.push({
						...
						text: `${slack.format.url(res.commit.url, slack.format.code(res.commit.short))} ${res.commit.message} - ${slack.format.bold(res.commit.branch.toUpperCase())}- ${res.commit.author}`,
						fields: [{value}],
						actions: [{
							type: 'button',
							text: 'Changes 🔍',
	...
	*/
	...
	checkRequest(ctx) {
		...
		let targetName = ctx.state.appName;
		// Run in branch variant if running else master
		if (git.branch !== 'master' && this.apps[`${targetName}-${git.branch}`]) {
			targetName = `${targetName}-${git.branch}`;
		}
		...
		if (!targetApp) {
			throw new Error('Target app not running');
		}
