...
class Worker {
	 ...
	 * @param {Boolean} checkRequest If we need to check the request validity (for manual triggering)
	 */
	handleRequest(ctx, checkRequest = true) {
		...
		let git;
		try {
			git = checkRequest ? this.checkRequest(targetApp, ctx) : undefined;
		}
		catch (error) {
