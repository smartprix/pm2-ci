		...
		const phases = {
			...
			testRunner: async () => {
				...
				if (!res.pass && targetApp.tests.deployAnyway) {
					logger.error(`Tests failed for app ${targetName} on commit ${res.commit.short}, found bad commit: ${res.bisect.commit.short}. Deploying anyway due to config`);
					return;
				}
				else if (!res.pass) {
					throw new Error(`Tests failed for app ${targetName} on commit ${res.commit.short}, found bad commit: ${res.bisect.commit.short}`);
				}
