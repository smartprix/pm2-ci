...
class Worker {
	 ...
	 * @param {Boolean} manualHook If we need to check the request validity/ignore DB (for manual triggering)
	 */
	handleRequest(ctx, manualHook = false) {
		...
		let git;
		try {
			git = manualHook ? undefined : this.checkRequest(targetApp, ctx);
		}
		catch (error) {
