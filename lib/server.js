...
// Basic parser
		...
		try {
			...
			if (urlPath.length >= 3 && /[a-f0-9]{40}/.test(urlPath[2])) {
				...
				if (urlPath.length >= 4 && urlPath[3] === 'coverage') {
					ctx.state.reportDir = 'coverage-reports';
					ctx.state.reportHtml = 'index.html';
				}
				else {
					ctx.state.reportDir = 'test-reports';
					ctx.state.reportHtml = `${ctx.state.appName}.html`;
				}
			}
			else {
	...
	app.use(async (ctx, next) => {
		...
		ctx.state.reportDir
			...
			const filePath = `${worker.opts.dataDir}/${ctx.state.reportDir}/` +
			`${ctx.state.appName}/${ctx.state.commit}/${ctx.state.reportHtml}`;
			...
			if (ctx.state.reportDir === 'coverage-reports') {
				await send(ctx, ctx.path.replace('/coverage', ''), {
					root: `${worker.opts.dataDir}/${ctx.state.reportDir}/`,
				});
			}
			else {
