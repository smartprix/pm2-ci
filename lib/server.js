...
// Basic parser
	app.use(async (ctx, next) => {
		try {
			...
			else if (['delete', 'edit', 'add', 'hook'].includes(urlPath[2])) {
	...
	app.use(async (ctx, next) => {
		if (ctx.state.post &&
		ctx.state.app &&
		ctx.state.action === 'delete'
		) {
			// console.log('deleting ', ctx.state.appName);
			const vars = new Vars(worker.opts.dataDir, ctx.state.appName)
			try {
				await db.deleteApp(vars.appName, vars.appDir);
				worker.opts.apps = await worker.constructor.getApps();
				worker.apps = worker.opts.apps;
				ctx.body = {
					msg: vars.appName + ' deleted!',
					status: true,
				}
			}
			catch(err) {
